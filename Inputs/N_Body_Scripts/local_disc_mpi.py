...
if __name__ == "__main__":
    ...
    output_directory =  '/Users/haydenmonk/Downloads/HPCC/Mass_Fraction/Outputs/Ejection_Results'

    output_file=output_directory + f"/{job_id}_ejection_results.txt"
    archive_filename='/Users/haydenmonk/Downloads/HPCC/Mass_Fraction/Outputs/Sim_Archives/' + f"{file_prefix}_sim.bin"
  ...
  #      0.1: 100_000,
    ...
    if np.random.randint(0,2):
        r_min = 1.0 + HR
        r_max = 1.0 + 2*np.sqrt(3)*HR
    else:
        r_min = 1.0 - 2*np.sqrt(3)*HR
    ...
    t_arr=np.arange(0,tmax,cadence)
    sim=create_sim(
        m_planet,
    ...
    for t in t_arr:
            ...
            if not ejected:
                ...
                if planet_centric_e < 1.0:
                    ...
                    if planet_centric_e_2 < 1.0:
                        write_results(output_file, m_planet, t, 'Captured')
                        ejected=True
        ...
        if rank ==0:
            ...
            if len(fraction_record) > slope_window:
                ...
                if slope>slope_peak[0]:
                    slope_peak=[slope, t]
            comparison_cadence=2*slope_peak[1]
            ...
            if len(fraction_record) > n_steps_back:
                difference = (
                    fraction_record[-1]
                    - fraction_record[-(n_steps_back + 1)]
                )
                if difference < 0.05:
                    stop=True
        else:
